// Copyright 2009 The Go Authors and David Roundy. All rights
...
// It is being modified by David Roundy to print C code rather than go
// code.  So far all I've done is to make it add some semicolons that
// are needed in C.
...
func (p *printer) signature(params *ast.FieldList) {
	p.parameters(params)
}
	...
	if isStruct {
		...
		for i, f := range list {
			...
			if ftyp, isFtyp := f.Type.(*ast.FuncType); isFtyp {
				// method
				p.funcreturn(ftyp.Results)
				p.expr(f.Names[0])
				p.signature(ftyp.Params)
			} else {
				// embedded interface
				p.expr(f.Type)
			}
...
func (p *printer) expr1(expr ast.Expr, prec1, depth int) {
	...
	case *ast.FuncType:
		p.funcreturn(x.Results)
		p.print(token.FUNC)
		p.signature(x.Params)
